...
class Bounds:
    ...
    def overlaps(self, other: "Bounds") -> bool:
        ...
        return not (
            self.north <= other.south or self.south >= other.north or self.east <= other.west or self.west >= other.east
        )
...
class TwoLayerElevationLibrary:
    ...
    def __init__(self, data_dir: str, resolution: int = 10):
        ...
        logger.info(f"Initialized TwoLayerElevationLibrary: {resolution}m resolution, " f"tile size {self.tile_size}°")
    ...
    def _ensure_layer1_coverage(self, bounds: Bounds) -> List[str]:
        ...
        for region_key in regions_to_download:
            ...
            try:
                ...
                dem = py3dep.get_dem(region_bounds.to_tuple(), resolution=self.resolution, crs="EPSG:4326")
        ...
        for row in range(min_row, max_row + 1):
            for col in range(min_col, max_col + 1):
                ...
                region_key = self._get_region_key(
                    (tile_bounds.north + tile_bounds.south) / 2, (tile_bounds.east + tile_bounds.west) / 2
                )
                ...
                region_file = os.path.join(self.layer1_dir, self.layer1_index["regions"][region_key]["file"])
    ...
    def load_area(self, bounds: Bounds) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Loading area: {bounds.south:.4f},{bounds.west:.4f} to " f"{bounds.north:.4f},{bounds.east:.4f}")
        ...
        try:
            ...
            return {"status": "error", "message": str(e), "tiles_created": 0}
    ...
    def remove_area(self, bounds: Bounds) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with status and statistics
        """
        logger.info(f"Removing area: {bounds.south:.4f},{bounds.west:.4f} to " f"{bounds.north:.4f},{bounds.east:.4f}")
    ...
    def remove_all(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "tiles_removed": tiles_removed, "regions_removed": regions_removed}
